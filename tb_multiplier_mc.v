module tb_multiplier_mc;
    ...
    initial begin
        $vcdplusfile("multiplier_mc.vpd");
        $vcdpluson(0, tb_multiplier_mc);
    end
    ...
    task run_test;
        ...
        begin
            ...
            if (valid !== 1'b1) begin
                $display("FAIL: timeout waiting for valid, a=%0d b=%0d",
                         test_a, test_b);
            end else if (elapsed_cycles !== 6) begin
                $display("FAIL: expected 6 cycles, got %0d cycles, a=%0d b=%0d",
                         elapsed_cycles, test_a, test_b);
            end else if (result !== expected) begin
                $display("FAIL: a=%0d b=%0d result=%0d expected=%0d cycles=%0d",
                         test_a, test_b, result, expected, elapsed_cycles);
            end else begin
                $display("PASS: a=%0d b=%0d result=%0d cycles=%0d",
                         test_a, test_b, result, elapsed_cycles);
            end
    ...
    initial begin
        ...
        $display("RTL multicycle simulation finished.");
        $vcdplusoff;
        $finish;
    end
